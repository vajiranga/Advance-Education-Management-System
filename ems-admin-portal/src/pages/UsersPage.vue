              ...
              >
                <template v-slot:top-right>
                  ...
                  <q-btn
                    ...
                    icon="add"
                    class="q-ml-md"
                    @click="openAddUserDialog('teacher')"
                  />
                </template>
                <template v-slot:body-cell-plain_password="props">
                  <q-td :props="props">
                    <div class="row items-center no-wrap">
                      <span v-if="visiblePasswords[props.row.id]">{{
                        props.row.plain_password
                      }}</span>
                      <span v-else>••••••••</span>
                      <q-btn
                        flat
                ...
                <template v-slot:top-right>
                  <div class="row q-gutter-sm">
                    ...
                    <q-btn
                      ...
                      label="Add New User"
                      icon="add"
                      @click="openAddUserDialog('student')"
                    />
                  </div>
                </template>
                <template v-slot:body-cell-plain_password="props">
              ...
              >
                <template v-slot:top-right>
                  ...
                  <q-btn
                    ...
                    icon="add"
                    class="q-ml-md"
                    @click="openAddUserDialog('parent')"
                  />
                </template>
                <template v-slot:body-cell-student_ids="props">
                  <q-td :props="props">
                    <div v-if="props.row.student_ids && props.row.student_ids.length > 0">
                      <q-badge
                        v-for="id in props.row.student_ids"
                        :key="id"
                        color="primary"
                        class="q-mr-xs"
                        >{{ id }}</q-badge
            ...
            <template v-slot:option="scope">
              <q-item v-bind="scope.itemProps">
                <q-item-section>
                  ...
                  <q-item-label caption v-if="scope.opt.teacher">{{
                    scope.opt.teacher.name
                  }}</q-item-label>
                </q-item-section>
              </q-item>
            </template>
          ...
          <q-form @submit="saveUser" class="q-gutter-md">
            ...
            <q-input
              ...
              :rules="
                isEditMode
                  ? []
                  : [(val) => !!val || 'Required', (val) => val.length >= 8 || 'Min 8 chars']
              "
            />
            ...
            <div class="row justify-end q-mt-md">
              ...
              <q-btn
                :label="isEditMode ? 'Update' : 'Save User'"
                type="submit"
                color="primary"
                :loading="loading"
              />
...
const parentCols = [
  { name: 'student_ids', label: 'Student IDs', align: 'left', field: 'student_ids' },
  { name: 'name', label: 'Name', align: 'left', field: 'name', sortable: true },
  { name: 'email', label: 'Email', align: 'left', field: 'email' },
  { name: 'phone', label: 'Phone', align: 'left', field: 'phone' },
  { name: 'actions', label: 'Actions', align: 'right' },
