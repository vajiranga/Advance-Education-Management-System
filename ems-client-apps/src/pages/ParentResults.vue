  ...
  <q-page :class="$q.dark.isActive ? 'q-pa-md bg-dark-page' : 'q-pa-md bg-grey-1'">
    ...
    <div class="row items-center justify-between q-mb-lg">
      <div class="row items-center q-gutter-x-md">
         ...
         <div class="text-subtitle1" :class="$q.dark.isActive ? 'text-grey-4' : 'text-grey-7'" v-if="selectedChild">
    ...
    <div class="row q-col-gutter-md q-mb-lg">
          ...
          <q-card class="no-shadow border-left-purple full-height" :class="$q.dark.isActive ? 'bg-dark' : 'bg-white'">
            <q-card-section>
               <div class="row items-center justify-between q-mb-md">
                   <div class="text-h6" :class="$q.dark.isActive ? 'text-white' : ''">Parent Meetings & Notices</div>
               ...
               <q-list separator>
                   <q-item v-for="notice in notices" :key="notice.id" class="q-px-none">
                       <q-item-section avatar>
                           <q-avatar :color="notice.type === 'meeting' ? 'orange' : 'teal'" text-color="white" :icon="notice.icon || 'campaign'" size="md" />
                       </q-item-section>
                       <q-item-section>
                           <q-item-label class="text-weight-bold" :class="$q.dark.isActive ? 'text-white' : ''">{{ notice.title }}</q-item-label>
                           <q-item-label caption :class="$q.dark.isActive ? 'text-grey-4' : ''">{{ notice.message }}</q-item-label>
                           <q-item-label caption class="text-xs text-grey" v-if="notice.course_name">{{ notice.course_name }} • {{ notice.teacher_name }}</q-item-label>
                       </q-item-section>
                       <q-item-section side top>
                           <q-item-label caption>{{ new Date(notice.date).toLocaleDateString() }}</q-item-label>
                       </q-item-section>
                   </q-item>
       ...
       <div class="col-12 col-md-4">
            <q-card class="text-center no-shadow full-height" :class="[$q.dark.isActive ? 'bg-dark text-green-2 border-dark' : 'bg-white text-purple-9 border-light']">
               <q-card-section>
      ...
      <q-card-section class="q-pt-none">
        ...
        >
          <template v-slot:body-cell-grade="props">
            <q-td :props="props">
              <q-badge :color="getGradeColor(props.row.grade)" :label="props.row.grade" />
            </q-td>
          </template>
            ...
            <q-td :props="props">
              <div class="row items-center justify-center q-gutter-x-xs">
                <q-icon :name="props.row.trend === 'up' ? 'trending_up' : 'trending_down'" 
                        :color="props.row.trend === 'up' ? 'green' : 'red'" size="sm" />
                <span :class="props.row.trend === 'up' ? 'text-green' : 'text-red'">{{ props.row.diff }}%</span>
              </div>
            </q-td>
          </template>
        </q-table>
